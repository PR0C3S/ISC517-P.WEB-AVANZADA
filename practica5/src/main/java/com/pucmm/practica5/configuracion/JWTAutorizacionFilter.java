...
public class JWTAutorizacionFilter extends OncePerRequestFilter {
    ...
    @Value("${token_jwt}")
    String SECRET = "grupo10_llave_secreta_asdasd_adadsasdas_asdasdasdas_1231231_asdasdsa_asdasdasdasd";

    @Override
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        if(request.getServletPath().startsWith("/jwt/auth")){
            filterChain.doFilter(request, response);
            return;
        }

        if(request.getServletPath().startsWith("/jwt/mock/view")){
